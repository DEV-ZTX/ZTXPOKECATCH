...
async def top_trainers(update: Update, context: CallbackContext) -> None:
...
async def global_top(update: Update, context: CallbackContext) -> None:
    """Shows the top 10 players with the most Pokémon collected globally."""
    ...
    leaderboard_message = "<b>🌍 GLOBAL TOP 10</b>\n\n"
    for i, user in enumerate(leaderboard_data, start=1):
        username = user.get('username', 'Unknown')
        first_name = html.escape(user.get('first_name', 'Unknown'))[:15] + '...'
        pokemon_count = user['pokemon_count']
...
async def send_groups_document(update: Update, context: CallbackContext) -> None:
    ...
    with open('groups.txt', 'w') as f:
        f.write("\n".join([group['group_id'] for group in groups]))
...
# Register Commands
application.add_handler(CommandHandler('top_trainers', top_trainers, block=False))  # Top 10 trainers in a group
...
application.add_handler(CommandHandler('global_top', global_top, block=False))  # Global top 10 players
